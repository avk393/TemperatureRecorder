...
void ADC0_InitTimer0ATriggerSeq3PD3(uint32_t period){
  ...
  ADC0_ACTSS_R |= 0x08;         // 10) enable sample sequencer 3
	ADC0_SAC_R = ADC_SAC_AVG_64X;	// enable hardware avging
...
void ADC0Seq3_Handler(void){
  ...
  //ADCvalue = ADC0_SSFIFO3_R;  // 12-bit result
	ADC_FIFO[ADC_counter] = ADC0_SSFIFO3_R;
	ADC_counter++;
	if(ADC_counter>=16) ADC_counter = 0;
}

uint32_t ADC_current_value(void){
	return ADC_FIFO[ADC_counter];
}
